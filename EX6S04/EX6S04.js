class CaixaRegistradora {
  ...
  atualizaEstoque() {
    const sobraEstoqueDiv = document.getElementById("sobraEstoque");
    sobraEstoqueDiv.innerHTML = "";
    const ul = document.createElement("ul");
    this.produtos.forEach((produto) => {
      const li = document.createElement("li");
      li.textContent = `Estoque atualizado do produto ${produto.nome}: ${produto.quantidadeEstoque}`;
      ul.appendChild(li);
    });
    sobraEstoqueDiv.appendChild(ul);
  }
...
function adicionarProdutoAoHTML(produto) {
  ...
  liEstoque.textContent = `Quantidade em estoque: ${produto.quantidadeEstoque}`;
  ul.appendChild(liEstoque);
...
document.getElementById("adicionarProdutoBtn").addEventListener("click", () => {
  ...
  const produto = caixa.adicionarProduto(codigoBarra, preco, nome, quantidadeEstoque);
  if (produto) {
...
document.getElementById("adicionarItemBtn").addEventListener("click", () => {
  ...
  if (adicionado) {
    ...
    caixa.atualizaEstoque();
  } else {
    document.getElementById("resultado").textContent = `Produto não encontrado ou não há quantidade suficiente.`;
  }
})
